...
並べ直しガイドの出し入れは `audio.ts` の `syncRearrangeGuide()` 1箇所で決めます。**`HighlightConfig.rearrangeGuide` は「上の句のあとに出す」ではなく単なる表示チェックです。** チェックが入っていれば基本は出したままで、フェーズが `shimo` / `silence` / `kami` の間だけ隠します。停止中・待機中・序歌の間は出たままになります（札を並べ直す時間なので）。読み上げの状態が変わる箇所とチェックの切り替えから呼んでください。
