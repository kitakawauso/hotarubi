...
`src/data.ts` の `computeEffectiveKimari(remainingIds, allPoems)` が中核です。CSV の初期決まり字ではなく、**まだ読まれていない札だけで一意に特定できる最小プレフィックス** を毎回計算し直します。読まれるほど決まり字は短くなります。
...
- `kimariji_stages` … 決まり字で段階的に絞り込む。プレフィックスに一致する場の札を候補色でまとめて光らせ、**決まり字を読み終えた最終段階で、かつ読まれた札が場にある（出札である）ときだけ**その1枚を確定色にする

**確定色は「出札が決まった瞬間」の意味です。** 場の一致が1枚に絞れただけで確定色にしてはいけません。決まり字の母集団は未読札全体なので、場では1枚でも未読の空札がまだ残っていることがあります。

空札が読まれたときも `highlightKarafuda` が有効なら同じ規則で光らせます。**ここで色分けを変えないこと。** 空札であることが見た目から分かってしまい、反応を測る意味がなくなります。空札のときは確定色に到達せず、決まり字が場のどの札とも違う長さまで進んだ時点で消えます。
...
並べ直しガイドの出し入れは `audio.ts` の `syncRearrangeGuide()` 1箇所で決めます。**`HighlightConfig.rearrangeGuide` は「上の句のあとに出す」ではなく単なる表示チェックです。** チェックが入っていて、かつ `_phase === null`（停止中・待機中）のときだけ出します。**序歌を含め、音が流れている間は隠します。** 読み上げの状態が変わる箇所とチェックの切り替えから呼んでください。フェーズを変える処理を足したら、そこからも呼ぶこと。
...
- **確定色を「場の一致が1枚になったとき」に出していた。** 決まり字の母集団は未読札全体なので、場の一致が1枚でも未読の空札が残っていることがあります。場に「なつ」「なにし」があるとき空札「なにわえ」を読むと、「なに」の時点で場の一致は「なにし」1枚になりますが、これは出札ではありません。ここで確定色を出すと、出札でない札を「取れ」と指示してしまいます。**確定色は出札かつ決まり字を読み終えた最終段階のときだけ。**
